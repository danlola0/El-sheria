...
if(isset($_GET['IdAss'])){
    ...
    $sql=$bd->query("SELECT * FROM ass WHERE IdAss=$Id");
    $result=$sql->fetch();
...
<head>
    ...
    <title>Modification Assistant(e)s</title>
    <style type="text/css">
    .gray {
        width: 100%;
        min-height: 100vh; 
...
<body>
    <section class="gray">
        <section><strong><h1>Modification des Assistant(e)s</h1></strong> 
...
$code=$bd->prepare("UPDATE ass SET Nom=? , Postnom=? , Prenom=? ,Tel=? , Email=? , Sexe=? , Article=? , Date=?, Pdf=?, FKIdProf=? , FKIdDep=? WHERE IdAss=$Id");
